...
def fetch_scraped_data(**kwargs):
    ...
    try:
        ...
        logging.error(f"Error fetching scraped data: {e}")
        raise

def clean_data(**kwargs):
    cleaner = DataCleaner()
    ti = kwargs['ti']
    news_data = ti.xcom_pull(task_ids='fetch_scraped_data_task', key='news_data')
    if not news_data:
    ...
    for record in news_data:
        try:
            ...
            logging.error(f"Error cleaning record with ID {record['id']}: {e}")
...
with DAG('news_cleaning_pipeline', default_args=default_args, schedule_interval='@daily', catchup=False) as dag:
    fetch_scraped_data_task = PythonOperator(
        ...
        provide_context=True,  # Deprecated, can be omitted in favor of op_kwargs
    )
